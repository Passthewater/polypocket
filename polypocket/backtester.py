...
available in bulk. Treat the results as directional, not exact live performance.
"""
...
def run_backtest(candles: list[dict], sigma_override: float | None = None) -> dict:
    ...
    for index in range(0, len(candles) - 4, 5):
        ...
        results.append(simulate_window(window_candles, sigma_5min=sigma))

    traded = [result for result in results if result.signal_fired]
    wins = [result for result in traded if result.pnl and result.pnl > 0]
    losses = [result for result in traded if result.pnl and result.pnl <= 0]
    total_pnl = sum(result.pnl for result in traded if result.pnl)
    ...
    return {
        ...
        "profit_factor": (
            sum(result.pnl for result in wins if result.pnl)
            / abs(sum(result.pnl for result in losses if result.pnl))
            if losses and any(result.pnl for result in losses)
            else float("inf")
        ),
        "max_consecutive_losses": _max_streak(traded, lambda result: result.pnl and result.pnl <= 0),
    }
